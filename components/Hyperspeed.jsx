...
const distortions = {
  mountainDistortion: {
    ...
    getJS: (progress, time) => {
      ...
      const distortion = new THREE.Vector3(Math.cos(progress * Math.PI * uFreq.x + time) * uAmp.x -
  ...
  LongRaceDistortion: {
    ...
    getJS: (progress, time) => {
      ...
      const distortion = new THREE.Vector3(Math.sin(progress * Math.PI * uFreq.x + time) * uAmp.x -
...
class CarLights {
  ...
  init() {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      }, this.webgl.fogUniforms, (typeof this.options.distortion === 'object' ? this.options.distortion.uniforms : {}) || {})
    });
  ...
  init() {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: Object.assign({
        uTravelLength: { value: options.length },
        uTime: { value: 0 }
      }, this.webgl.fogUniforms, (typeof options.distortion === 'object' ? options.distortion.uniforms : {}) || {})
    });
